...
**How to read the picture.** The steepest-descent direction $-\nabla f(x_k)$ is drawn twice:
in <span style="color:#6b6b6b;">**grey**</span> with a fixed length, showing direction only,
and in <span style="color:#E8820C;">**orange**</span> at its actual length $\|\nabla f(x_k)\|$.
The <span style="color:#2E8B57;">**green**</span> arrow is the step actually taken,
$\alpha_k d_k$, from $x_k$ to $x_{k+1}$. The angle between grey and green is what
distinguishes the four algorithms; the orange arrow shows why a raw gradient step is
unusable as a step length. On the Rosenbrock ridge $\|\nabla f\|$ reaches several hundred,
so the orange arrow runs off the frame and is clipped — that is the point being made.

The slider plays once through the iterations and stops. **Next step** and **Previous**
advance it by one iteration for discussion at the board.
...
server <- function(input, output, session) {
  ...
  output$contour <- renderPlot({
    ...
    L <- 0.09 * diff(FNc$xlim)                      # fixed display length
    if (gn > 0) {
      u <- -gk / gn
    ...
    legend("topleft", bty = "n", lwd = 3, cex = 0.9,
...
2.  On the Rosenbrock ridge, the gradient points almost perpendicular to the valley,
    so gradient descent makes little progress per iteration. Newton and BFGS turn the step
    along the valley; the angle between the grey and green arrows becomes large.
