...
```{r}
#| label: perm-lm-sel-compare
rbind(test_lm_sel = c(size = mean(pv_sel0 < 0.05),  power = mean(pv_sel1 < 0.05)),
      permutation = c(size = mean(pv_perm0 < 0.05), power = mean(pv_perm1 < 0.05)))
```

- `test_lm_sel` has a higher rejection rate under $H_1$ only because it also rejects far too often under $H_0$.
