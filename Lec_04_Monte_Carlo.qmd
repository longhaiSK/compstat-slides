...
# Shinylive App: Inverse CDF (Survival) Sampling
...
dists <- list(
  ...
  "Piecewise constant (1 : 5 : 10)" = list(
    ...
    mean = sum(pw_h / 3 * (pw_b[-4] + pw_b[-1]) / 2)
  ),
  "Binomial(5, 0.5)" = list(
    discrete = TRUE,
    xmax = 5,
    p    = function(x) pbinom(x, size = 5, prob = 0.5),
    qinv = function(u) qbinom(u, size = 5, prob = 0.5),
    dens = function() {
      xs <- 0:5
      segments(xs, 0, xs, dbinom(xs, 5, 0.5), col = "steelblue", lwd = 2.5)
      points(xs, dbinom(xs, 5, 0.5), pch = 16, col = "steelblue", cex = 1.2)
    },
    hist_breaks = function() seq(-0.5, 5.5, by = 1),
    mean = 5 * 0.5
  )
)
...
server <- function(input, output, session) {
  ...
  ## switching the target distribution or the CDF/survival toggle restarts
  ## the sampling, since old draws are shown against the newly chosen curve
  ...
  output$cdfPlot <- renderPlot({
    ...
    par(mar = c(4, 4, 2.5, 1))
    plot(g, yvals, type = "n", ylim = c(0, 1), xlab = "x", ylab = ylab, main = main)
  ...
  output$histPlot <- renderPlot({
    ...
    brk    <- D()$hist_breaks()
    disc   <- D()$discrete
    ylab   <- if (disc) "proportion" else "density"
    main   <- if (disc) "Accumulated draws vs. the true pmf"
              else "Accumulated draws vs. the true density"
    hist(rv$x, breaks = brk, freq = FALSE, col = "grey85", border = "white",
         xlim = c(0, xmax), xlab = "x", ylab = ylab, main = main)
    D()$dens()
    abline(v = mean(rv$x), col = "firebrick", lwd = 2, lty = 2)
    legend("topright", bty = "n", lwd = 2, lty = c(1, 2),
           col = c("steelblue", "firebrick"),
           legend = c(if (disc) "true pmf" else "true density", "sample mean"))
  })
}
