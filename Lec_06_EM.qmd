...
## Kullback-Leibler Distance {.smaller}

**Definition**: for densities $P(z)$ and $q(z)$ on the same space,
$$KL(P\|q) = \int P(z)\log\frac{P(z)}{q(z)}\,dz = E_P\!\left[\log\frac{P(Z)}{q(Z)}\right]$$



**Theorem (Gibbs' inequality)**: $KL(P\|q) \ge 0$, with equality iff $P = q$ (a.e.)

**Proof.** Since $\log x \le x - 1$ (equality iff $x=1$),
$$
\begin{aligned}
KL(P\|q) &= -\int P(z)\log\frac{q(z)}{P(z)}\,dz \ \ge\ -\int P(z)\Big(\frac{q(z)}{P(z)}-1\Big)\,dz \\
&= -\int\big[q(z)-P(z)\big]\,dz = 0 \qquad\blacksquare
\end{aligned}
$$
**Remarks:**
...
mu_p <- 0; sd_p <- 1; mu_q <- 1.2; sd_q <- 1.6; nsamp <- 40
x  <- rnorm(nsamp, mu_p, sd_p)                   # samples from P
lp <- dnorm(x, mu_p, sd_p, log = TRUE)
lq <- dnorm(x, mu_q, sd_q, log = TRUE)

curve(dnorm(x, mu_p, sd_p), -4, 6, lwd = 3, col = "steelblue", xlab = "x", ylab = "density",
      main = "P(x), q(x), and the samples")
curve(dnorm(x, mu_q, sd_q), add = TRUE, lwd = 3, col = "firebrick")
rug(x, col = "gray30", lwd = 1.5)
legend("topright", bty = "n", lwd = 3, col = c("steelblue", "firebrick"),
       legend = c(expression(P(x)), expression(q(x))))

ord <- order(x)
plot(x[ord], lp[ord], type = "b", pch = 19, col = "steelblue", lwd = 2,
     xlab = "x (sorted)", ylab = "log-density at sample", ylim = range(c(lp, lq)),
     main = expression(log~P(x[i])~vs.~log~q(x[i])))
lines(x[ord], lq[ord], type = "b", pch = 19, col = "firebrick", lwd = 2)
segments(x[ord], lp[ord], x[ord], lq[ord], col = "gray70")
abline(h = mean(lp), col = "steelblue", lty = 2, lwd = 2)
abline(h = mean(lq), col = "firebrick", lty = 2, lwd = 2)
legend("bottomleft", bty = "n", lwd = 2, lty = c(1, 1, 2, 2), pch = c(19, 19, NA, NA),
       col = c("steelblue", "firebrick", "steelblue", "firebrick"),
       legend = c(expression(log~P(x[i])), expression(log~q(x[i])),
                  bquote(mean(log~P(x[i])) == .(round(mean(lp), 2))),
                  bquote(mean(log~q(x[i])) == .(round(mean(lq), 2)))))
```
...
## Formal Proof {style="font-size: 30px"}

**Proof.** Set $\theta_0 = \hat\theta^{(t)}$ and subtract the identity $\ell_{\text{obs}}(\theta) = Q(\theta\mid\hat\theta^{(t)}) - H(\theta\mid\hat\theta^{(t)})$ evaluated at $\theta = \hat\theta^{(t)}$ from the same identity at $\theta = \hat\theta^{(t+1)}$:
$$
\begin{aligned}
\ell_{\text{obs}}(\hat\theta^{(t+1)}) - \ell_{\text{obs}}(\hat\theta^{(t)})
&= \big[Q(\hat\theta^{(t+1)}\mid\hat\theta^{(t)}) - Q(\hat\theta^{(t)}\mid\hat\theta^{(t)})\big]
- \big[H(\hat\theta^{(t+1)}\mid\hat\theta^{(t)}) - H(\hat\theta^{(t)}\mid\hat\theta^{(t)})\big] \\[2mm]
&= \underbrace{\big[Q(\hat\theta^{(t+1)}\mid\hat\theta^{(t)}) - Q(\hat\theta^{(t)}\mid\hat\theta^{(t)})\big]}_{\ge 0 \text{ by the M-step}}
+ \underbrace{KL\big(K(\cdot\mid Y,\hat\theta^{(t)})\,\big\|\,K(\cdot\mid Y,\hat\theta^{(t+1)})\big)}_{\ge 0 \text{ by Gibbs' inequality}} \ \ge\ 0,
\end{aligned}
$$
where the second line uses
$$
H(\hat\theta^{(t)}\mid\hat\theta^{(t)}) - H(\theta\mid\hat\theta^{(t)})
= E_{\hat\theta^{(t)}}\!\left[\log \frac{K(Z\mid Y,\hat\theta^{(t)})}{K(Z\mid Y,\theta)} \,\Big|\, Y\right]
= KL\big(K(\cdot\mid Y,\hat\theta^{(t)})\,\big\|\,K(\cdot\mid Y,\theta)\big)
$$
at $\theta = \hat\theta^{(t+1)}$. $\blacksquare$
...
$$f(x\mid Y=1) = \frac{f(x)\,I(x>c)}{\int_c^\infty f(t)\,dt}, \qquad
