...
## Formal Proof: The $H$ Term via Gibbs' Inequality

**Proof.** Set $\theta_0 = \hat\theta^{(t)}$ and subtract the identity at $\theta = \hat\theta^{(t)}$ from that at $\theta = \hat\theta^{(t+1)}$. The dropped term is exactly a KL divergence, $H(\hat\theta^{(t)}\mid\hat\theta^{(t)}) - H(\theta\mid\hat\theta^{(t)}) = KL\big(K(\cdot\mid Y,\hat\theta^{(t)})\,\|\,K(\cdot\mid Y,\theta)\big)$, so
$$
\begin{aligned}
...
&\quad + \underbrace{KL\big(K(\cdot\mid Y,\hat\theta^{(t)})\,\|\,K(\cdot\mid Y,\hat\theta^{(t+1)})\big)}_{\ge 0 \text{ by Gibbs' inequality}} \ \ge\ 0. \qquad\blacksquare
\end{aligned}
$$
