...
## ------------------------------------------------------------------- data
sim_data <- function(n, sigma, seed = 1) {
  set.seed(seed)
  x <- runif(n, 1, 3)                       # not centred: b0 and b1 correlate
  y <- 1 + 2 * x + rnorm(n, 0, sigma)
  list(x = x, y = y, n = n)
}

suff <- function(D)
  list(n = D$n, Sx = sum(D$x), Sy = sum(D$y), Sxx = sum(D$x^2),
       Sxy = sum(D$x * D$y), Syy = sum(D$y^2))

## full-sample loss F(b) = sum of squares / (2n), evaluated on a grid
loss_grid <- function(S, b0s, b1s)
  outer(b0s, b1s, function(a, b)
    (S$Syy - 2 * a * S$Sy - 2 * b * S$Sxy + S$n * a^2 +
     2 * a * b * S$Sx + b^2 * S$Sxx) / (2 * S$n))

loss_full <- function(S, b)
  (S$Syy - 2 * b[1] * S$Sy - 2 * b[2] * S$Sxy + S$n * b[1]^2 +
   2 * b[1] * b[2] * S$Sx + b[2]^2 * S$Sxx) / (2 * S$n)

grad_sub <- function(D, b, idx) {                 # gradient on a subset
  x <- D$x[idx]; y <- D$y[idx]
  r <- y - b[1] - b[2] * x
  -c(mean(r), mean(x * r))
}
...
## -------------------------------------------------------------------- SGD
run_sgd <- function(D, b0, m, a0, decay, niter, seed) {
  S <- suff(D); set.seed(seed)
  B <- matrix(NA_real_, niter + 1, 2)
  GB <- matrix(NA_real_, niter + 1, 2)             # minibatch gradient
  GF <- matrix(NA_real_, niter + 1, 2)             # full gradient
  AL <- rep(NA_real_, niter + 1); FL <- rep(NA_real_, niter + 1)
  IDX <- vector("list", niter + 1)
  b <- b0
  for (k in seq_len(niter + 1)) {
    idx <- sample.int(D$n, m)
    B[k, ] <- b; IDX[[k]] <- idx
    GB[k, ] <- grad_sub(D, b, idx)
    GF[k, ] <- grad_sub(D, b, seq_len(D$n))
    FL[k] <- loss_full(S, b)
    if (k > niter) break
    a <- if (decay) a0 / (1 + (k - 1) / 20) else a0
    AL[k] <- a
    b <- b - a * GB[k, ]
    if (any(!is.finite(b))) { B <- B[1:k, , drop = FALSE]; break }
  }
  n <- sum(is.finite(B[, 1]))
  list(B = B[1:n, , drop = FALSE], GB = GB[1:n, , drop = FALSE],
       GF = GF[1:n, , drop = FALSE], A = AL[1:n], FL = FL[1:n],
       IDX = IDX[1:n], n = n)
}

run_gd <- function(D, b0, a0, niter) {             # full-batch reference
  S <- suff(D); b <- b0
  B <- matrix(NA_real_, niter + 1, 2); FL <- rep(NA_real_, niter + 1)
  for (k in seq_len(niter + 1)) {
    B[k, ] <- b; FL[k] <- loss_full(S, b)
    if (k > niter) break
    b <- b - a0 * grad_sub(D, b, seq_len(D$n))
    if (any(!is.finite(b))) break
  }
  n <- sum(is.finite(B[, 1]))
  list(B = B[1:n, , drop = FALSE], FL = FL[1:n], n = n)
}

expand_box <- function(xlim, ylim, pin) {
  if (length(pin) != 2 || any(!is.finite(pin)) || any(pin <= 0))
    return(list(xlim = xlim, ylim = ylim))
  rr <- pin[1] / pin[2]; wd <- diff(xlim); ht <- diff(ylim)
  if (wd / ht < rr) { w <- ht * rr; m <- mean(xlim); xlim <- m + c(-.5, .5) * w }
  else { h <- wd / rr; m <- mean(ylim); ylim <- m + c(-.5, .5) * h }
  list(xlim = xlim, ylim = ylim)
}

BOX <- list(b0 = c(-4, 6), b1 = c(-1, 5))
START <- c(-3, 4.5)
...
ui <- fluidPage(
  fluidRow(
    column(
      3,
      sliderInput("m", "Minibatch size m", min = 1, max = 100, value = 5,
                  step = 1),
      sliderInput("a0", "Learning rate", min = 0.005, max = 0.30,
                  value = 0.06, step = 0.005),
      checkboxInput("decay", "Decaying rate a0 / (1 + k/20)", FALSE),
      sliderInput("k", "Iteration k", min = 0, max = 1, value = 0, step = 1),
      checkboxInput("showpath", "Keep the whole path", TRUE),
      checkboxInput("showgd", "Overlay full-batch gradient descent", TRUE),
      htmlOutput("info")
    ),
    column(
      2,
      div(
        ...
        actionButton("reset", "Reset initial",
                     style = "width:100%; margin-bottom:8px;"),
        actionButton("newseed", "New sample path",
                     style = "width:100%; margin-bottom:8px;"),
        helpText("Click inside the contour plot to choose a new initial value.")
      )
    ),
...
server <- function(input, output, session) {
  ...
  D <- sim_data(n = 200, sigma = 1, seed = 11)
  S <- suff(D)
  bhat <- solve(matrix(c(S$n, S$Sx, S$Sx, S$Sxx), 2, 2), c(S$Sy, S$Sxy))
  Fmin <- loss_full(S, bhat)

  start <- reactiveVal(START)
  seed <- reactiveVal(1)
  playing <- reactiveVal(FALSE)
  NIT <- 120

  observeEvent(input$reset, start(START))
  observeEvent(input$newseed, seed(seed() + 1))
  observeEvent(input$click, {
    p <- c(input$click$x, input$click$y)
    if (all(is.finite(p))) start(p)
  })
  ...
  path <- reactive({
    req(start())
    run_sgd(D, start(), input$m, input$a0, isTRUE(input$decay), NIT, seed())
  })
  gpath <- reactive(run_gd(D, start(), input$a0, NIT))
  ...
  output$contour <- renderPlot({
    ...
    par(mar = c(4, 4, 2.5, 1))
    plot.new()
    bx <- expand_box(BOX$b0, BOX$b1, par("pin"))
    plot.window(xlim = bx$xlim, ylim = bx$ylim, xaxs = "i", yaxs = "i")

    b0s <- seq(bx$xlim[1], bx$xlim[2], length.out = 200)
    b1s <- seq(bx$ylim[1], bx$ylim[2], length.out = 200)
    z <- loss_grid(S, b0s, b1s)
    lv <- unique(quantile(z, probs = seq(0, 1, length.out = 30)^1.7))
    zr <- matrix(rank(z, ties.method = "average"), nrow = length(b0s))
    image(b0s, b1s, zr, add = TRUE,
          col = colorRampPalette(c("#f7fbff", "#9ec4e3"))(128))
    contour(b0s, b1s, z, levels = lv, add = TRUE, col = "grey45",
            drawlabels = FALSE)
    axis(1); axis(2); box()
    title(main = paste0("SGD on least squares:  m = ", input$m,
                        ",  rate = ", format(P$A[min(k, P$n - 1)], digits = 3)),
          xlab = expression(b[0]), ylab = expression(b[1]))
    points(bhat[1], bhat[2], pch = 8, col = "red", cex = 1.4, lwd = 2)

    if (isTRUE(input$showgd)) {
      G <- gpath(); kg <- min(k, G$n)
      lines(G$B[1:kg, 1], G$B[1:kg, 2], col = "#7B3FBF", lwd = 2)
    }
    if (isTRUE(input$showpath) && k > 1)
      lines(P$B[1:k, 1], P$B[1:k, 2], col = "grey25", lty = 1)
    points(P$B[1:k, 1], P$B[1:k, 2], pch = 20, col = "grey40", cex = 0.7)

    bk <- P$B[k, ]; gf <- P$GF[k, ]; gb <- P$GB[k, ]
    L <- 0.10 * diff(bx$xlim)
    if (sqrt(sum(gf^2)) > 0) {
      u <- -gf / sqrt(sum(gf^2))
      arrows(bk[1], bk[2], bk[1] + L * u[1], bk[2] + L * u[2],
             col = "grey35", lwd = 3, length = 0.10)
    }
    if (sqrt(sum(gb^2)) > 0) {
      u <- -gb / sqrt(sum(gb^2))
      arrows(bk[1], bk[2], bk[1] + L * u[1], bk[2] + L * u[2],
             col = "#E8820C", lwd = 3, length = 0.10)
    }
    if (k < P$n)
      arrows(bk[1], bk[2], P$B[k + 1, 1], P$B[k + 1, 2],
             col = "#2E8B57", lwd = 3, length = 0.12)
    points(bk[1], bk[2], pch = 19, cex = 1.3)

    legend("topright", cex = 0.85, lwd = 3, bg = "#ffffffcc", box.col = NA,
           col = c("grey35", "#E8820C", "#2E8B57", "#7B3FBF"),
           legend = c("full gradient (direction)",
                      "minibatch gradient (direction)",
                      "step taken", "full-batch GD path"))
  })
  ...
  output$side <- renderPlot({
    ...
    idx <- P$IDX[[k]]
    plot(D$x, D$y, pch = 20, col = "grey70", xlab = "x", ylab = "y",
         main = "data and current fit")
    points(D$x[idx], D$y[idx], pch = 19, col = "#E8820C", cex = 1.2)
    abline(a = bhat[1], b = bhat[2], col = "red", lty = 2, lwd = 2)
    abline(a = P$B[k, 1], b = P$B[k, 2], col = "#2E8B57", lwd = 2.5)

    ex <- pmax(P$FL - Fmin, 1e-12)
    plot(seq_len(P$n) - 1, ex, type = "l", log = "y", col = "grey40",
         xlab = "iteration k", ylab = expression(F(b[k]) - F(hat(b))),
         main = "excess loss")
    if (isTRUE(input$showgd)) {
      G <- gpath()
      lines(seq_len(G$n) - 1, pmax(G$FL - Fmin, 1e-12), col = "#7B3FBF", lwd = 2)
    }
    points(k - 1, ex[k], pch = 19, col = "#E8820C", cex = 1.5)
  })

  output$info <- renderUI({
    P <- path(); k <- min(as.integer(input$k), P$n - 1) + 1L
    gf <- P$GF[k, ]; gb <- P$GB[k, ]
    cs <- sum(gf * gb) / (sqrt(sum(gf^2)) * sqrt(sum(gb^2)))
    ang <- acos(max(-1, min(1, cs))) * 180 / pi
    fmt <- function(v, d = 4) formatC(v, format = "g", digits = d)
    HTML(paste0(
      "<hr><b>iteration</b> ", k - 1, " of ", P$n - 1, "<br>",
      "<b>b<sub>k</sub></b> = (", fmt(P$B[k, 1]), ", ", fmt(P$B[k, 2]), ")<br>",
      "<b>F(b<sub>k</sub>)</b> = ", fmt(P$FL[k], 6), "<br>",
      "<b>excess loss</b> = ", fmt(P$FL[k] - Fmin), "<br><br>",
      "<b>||full grad||</b> = ", fmt(sqrt(sum(gf^2))), "<br>",
      "<b>||batch grad||</b> = ", fmt(sqrt(sum(gb^2))), "<br>",
      "<b>angle between them</b> = ", fmt(ang, 3), "&deg;<br><br>",
      "<i>least squares solution: (", fmt(bhat[1]), ", ", fmt(bhat[2]), ")</i>"
    ))
  })
}
