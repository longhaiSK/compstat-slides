...
## -------------------------------------------------------------------
## ------------------------------------------------------------------- data
## True DGP is itself a 1 - 3 tanh - 1 network ("teacher"), so a student
## network with H >= 3 can represent it exactly.
...
make_data <- function(n, sigma, seed) {
  set.seed(500 + seed)
  x <- sort(runif(n, -2, 2))
  list(x = x, y = gtrue(x) + rnorm(n, 0, sigma), n = n)
}
...
## ---------------------------------------------------------------- training
## B = number of batches per epoch; B = 1 is full-batch gradient descent.
train <- function(p0, H, B, a, nepoch, seed, D) {
  n <- D$n
  nup <- B * nepoch
  ...
  for (e in seq_len(nepoch)) {
    ...
    grp <- split(perm, rep_len(seq_len(B), n))       # nearly equal sizes
    for (b in seq_len(B)) {
      k <- k + 1
      idx <- grp[[b]]
      P[k, ] <- p; IDX[[k]] <- idx
  ...
  list(P = P[1:n_rec, , drop = FALSE], L = L[1:n_rec], EP = EP[1:n_rec],
       IDX = IDX[1:n_rec], n = n_rec, B = B, diverged = bad)
}

## ------------------------------------------------------------ weight plot
HOT <- colorRampPalette(c("#f0f0f0", "#ffe08a", "#ffa73c",
                          "#ef4a15", "#7f0000"))(100)
wcol <- function(v, wmax)
  HOT[pmax(1, pmin(100, ceiling(abs(v) / wmax * 100)))]

draw_net <- function(p, H, wmax, main) {
  P <- unpack(p, H)
  yh <- if (H == 1) 0.5 else seq(0.92, 0.10, length.out = H)
  ix <- 0.05; hx <- 0.55; ox <- 1.05
  nc <- max(1.2, min(4.2, 34 / H))                   # node size shrinks with H
  plot.new(); plot.window(c(-0.05, 1.2), c(-0.08, 1.02))
  title(main = main, cex.main = 1.05)
  lw <- function(v) 0.7 + 4.5 * abs(v) / wmax
  for (j in 1:H)
    segments(ix, 0.5, hx, yh[j], col = wcol(P$w[j], wmax),
             lwd = lw(P$w[j]), lty = if (P$w[j] < 0) 2 else 1)
  for (j in 1:H)
    segments(hx, yh[j], ox, 0.5, col = wcol(P$v[j], wmax),
             lwd = lw(P$v[j]), lty = if (P$v[j] < 0) 2 else 1)
  points(ix, 0.5, pch = 21, cex = 3.2, bg = "white", lwd = 2)
  text(ix, 0.5, "x", font = 2)
  points(rep(hx, H), yh, pch = 21, cex = nc, lwd = 1.6, bg = wcol(P$cc, wmax))
  if (H <= 8) text(rep(hx, H), yh, paste0("h", 1:H), cex = 0.7)
  points(ox, 0.5, pch = 21, cex = 3.4, lwd = 2, bg = wcol(P$b, wmax))
  text(ox, 0.5, "y", font = 2)
  xs <- seq(0.15, 0.95, length.out = 100)
  rect(xs, -0.07, xs + diff(xs)[1], -0.02, col = HOT, border = NA)
  text(0.15, 0.005, "0", cex = 0.75)
  text(0.95, 0.005, formatC(wmax, format = "g", digits = 3), cex = 0.75)
  text(0.55, 0.005, "|weight|", cex = 0.75)
}
  ...
  fluidRow(
    ...
    column(
      ...
      sliderInput("H", "Hidden neurons", min = 2, max = 16, value = 8,
                  step = 1),
      sliderInput("B", "Number of batches B per epoch", min = 1, max = 40,
                  value = 10, step = 1),
      ...
      checkboxInput("showgd", "Overlay the B = 1 run", TRUE),
      htmlOutput("info")
    ),
    ...
    column(
      ...
      div(
        ...
        helpText("Both runs share the data, the initial weights and the",
                 "number of epochs; only B differs.")
      )
    ),
    ## ---- column 3: plots -----------------------------------------------
    column(
...
server <- function(input, output, session) {
  ...
  DAT <- reactive(make_data(input$n, input$sigma, dseed()))

  runs <- reactive({
    D <- DAT(); H <- input$H; B <- as.integer(input$B)
    ne <- max(1, min(input$nep, floor(MAXUP / B)))
    p0 <- init_p(H, wseed())
    S <- train(p0, H, B, input$a, ne, bseed(), D)
    G <- train(p0, H, 1L, input$a, ne, bseed(), D)
    list(S = S, G = G, H = H, B = B, D = D, ne = ne,
         capped = ne < input$nep,
         wmax = max(1e-6, max(abs(S$P), na.rm = TRUE),
                    max(abs(G$P), na.rm = TRUE)))
  })

  stride <- reactive(if (isTRUE(input$byepoch)) as.integer(input$B) else 1L)
  ...
  output$nets <- renderPlot({
    ...
    draw_net(R$S$P[C$k, ], R$H, R$wmax,
             sprintf("B = %d (m = %d),  epoch %.2f", R$B, msz, C$ep))
    if (isTRUE(input$showgd))
      draw_net(R$G$P[C$kg, ], R$H, R$wmax,
               sprintf("B = 1 (m = %d),  epoch %d", R$D$n, C$kg - 1L))
  })
  ...
  output$fit <- renderPlot({
    ...
    lines(XG, gtrue(XG), col = "red", lty = 2, lwd = 2)
    ...
    legend("topright", bty = "n", cex = 0.85,
           ...
           legend = c("true curve", paste0("B = ", R$B, " fit"), "B = 1 fit"))
  ...
  output$info <- renderUI({
    ...
    HTML(paste0(
      ...
      "<b>loss, B = ", R$B, "</b> = ", fmt(R$S$L[C$k], 5), "<br>",
      "<b>loss, B = 1</b> = ", fmt(R$G$L[C$kg], 5), "<br>",
      "<b>noise floor</b> &asymp; ", fmt(0.5 * input$sigma^2, 5), "<br>",
      if (isTRUE(R$S$diverged))
        "<b style='color:#C81E1E'>B run diverged</b><br>" else "",
      if (isTRUE(R$capped))
