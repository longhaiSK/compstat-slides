...
<!-- Replaces "## Fixed-Point (Simple) Iteration" and "## Fixed-Point: Two Rearrangements".
     Depends on the ztp-setup chunk: ll, S, lam, lhat, xbar. -->

## Fixed-Point Iteration: Two Rearrangements {.smaller}

The score equation $S(\lambda) = 0$ can be written as
$$
...
$$
\lambda = \overline x\,(1 - e^{-\lambda}) =: g_1(\lambda),
\qquad
\lambda = -\log\!\Big(1 - \frac{\lambda}{\overline x}\Big) =: g_2(\lambda) .
$$
...
Pick a rearrangement $\lambda = g(\lambda)$ and iterate
$$
\lambda_{k+1} = g(\lambda_k), \qquad k = 0, 1, 2, \ldots
$$
...
g1 <- function(l) xbar * (1 - exp(-l))
g2 <- function(l) -log(1 - l / xbar)
fixed_point(g1, x0 = xbar)
c(g1_slope = xbar * exp(-lhat), g2_slope = 1 / (xbar - lhat))    # reciprocals
```

Advantage: no derivatives. Disadvantage: a suitable $g$ must be found, and the rearrangement alone decides the outcome.
...
cobweb(g2, x0 = 1.40, nstep = 6, main = TeX("$g_2$: repelled, $|g_2'| > 1$"))
```
...
legend("bottomleft", bty = "n", lwd = 2, lty = c(1, 1, 5), pch = c(19, 19, NA),
       col = c(1, 2, 4), legend = c(TeX("$g_1$"), TeX("$g_2$"), TeX("rate $|g_1'(\\hat{\\lambda})|^k$")))
```
...
l0 <- 3.0; l1 <- l0 - S(l0) / dS(l0)
plot(lam, S(lam), type = "l", lwd = 2, main = "one Newton step",
     xlab = TeX("$\\lambda$"), ylab = TeX("$S(\\lambda)$"))
abline(h = 0, lty = 3); points(lhat, 0, pch = 19, cex = 1.2)
xt <- c(l1 - 0.3, l0 + 0.3)                      # draw the whole tangent, not just the step
lines(xt, S(l0) + dS(l0) * (xt - l0), col = 2, lwd = 2)
points(c(l0, l1), c(S(l0), 0), pch = 19, col = 2)
segments(l1, 0, l1, S(l1), lty = 3, col = "grey40")
text(l0, S(l0), TeX("$\\lambda_k$"), pos = 4)
...
newton(S, dS, x0 = xbar, trace = TRUE)
```

Iteration counts on the same problem: fixed point `r fixed_point(g1, xbar)["iter"]`, bisection 30, golden 40, Newton `r newton(S, dS, xbar)["iter"]`.
...
c(lambda_1 = l_nr)
tryCatch(newton(S, dS, l0), error = function(e) conditionMessage(e))
```

Safeguards: start from a robust estimate, **halve the step** until $\ell$ increases, or fall back to bisection/golden section, which only need a bracket.
...
rbind(newton = newton(S, dS, 3), scoring = fisher_scoring(S, nI, 3))
```
...
```{r}
#| echo: false
nr_path <- mle_logistic_nr(c(0, 0), 8, r2, z2)     # beta0, beta1, neg_loglike per Newton iteration
